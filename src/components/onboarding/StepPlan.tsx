  ...
  return (
    <div className="space-y-6">
        ...
        {OPTIONS.map(({ code, recommended }) => {
          ...
          return (
            <button
              ...
              className={`flex w-full items-center gap-3 rounded-[5px] border-2 p-4 text-left transition-colors ${
                isSel ? "border-brand bg-brand-50" : "border-gray-200 bg-white hover:border-gray-300"
              }`}
            >
              <span className={`flex h-5 w-5 flex-shrink-0 items-center justify-center rounded-[4px] border-2 ${isSel ? "border-brand bg-brand text-white" : "border-gray-300"}`}>
                {isSel && <Check className="h-3 w-3" />}
              </span>
              <span className="min-w-0 flex-1">
                <span className="flex items-center gap-2">
                  <span className="text-sm font-bold text-gray-900">{plan.displayName}</span>
                  {recommended && <span className="rounded-[3px] bg-amber-100 px-2 py-0.5 text-[10px] font-bold text-amber-700">RECOMMENDED</span>}
                </span>
      ...
      <div className="flex items-center gap-3">
        ...
        <button onClick={submit} disabled={busy} className="flex-1 rounded-[4px] bg-brand py-3 text-base font-semibold text-white hover:bg-brand-600 disabled:opacity-60">
