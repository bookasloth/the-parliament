...
/**
 * Textarea/input with @mention autocomplete. When the user types "@…" the
 * ranked suggestion list (follow-graph → popular → alphabetical) drops in;
 * picking a row inserts "@username ". Used by both the comment composer and
...
export default function MentionInput({
  ...
  // Detect the @token under the caret and fetch suggestions (debounced).
  useEffect(() => {
    ...
    tokenStart.current = m.index! + m[1].length // position of the "@"
    const q = m[2]
    let live = true
