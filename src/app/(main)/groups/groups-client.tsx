...
export default function GroupsClient({ groups = MOCK_GROUPS }: { groups?: Group[] }) {
  ...
  const toggleJoin = (id: string) => {
    const willJoin = !joinedGroups.includes(id)
    setJoinedGroups(prev => willJoin ? [...prev, id] : prev.filter(x => x !== id))
    // Fire-and-forget server action; mock groups (non-UUID ids) will simply no-op on the server.
    if (willJoin) {
      void joinGroupAction(id).catch(() => {})
    } else {
      void leaveGroupAction(id).catch(() => {})
    }
  }
