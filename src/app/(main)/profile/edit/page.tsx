...
export default async function EditProfilePage() {
  ...
  const [user, facets, experiences, educations] = await Promise.all([
    prisma.user.findUnique({
      ...
      select: {
        ...
        dateOfBirth: true, gender: true, mobileE164: true, membershipStatus: true,
        profile: {
          select: {
            photoUrl: true, coverUrl: true, bio: true, headline: true,
            houseId: true, batchId: true, bloodGroup: true, bloodDonor: true,
            city: true, correspondenceAddress: true, homeTown: true, whatsappOptIn: true,
            company: true, designation: true, industry: true, department: true, workSince: true,
            higherEducation: true, skills: true,
            linkedinUrl: true, socialLinks: true, visibility: true, showOnMap: true, isPublicIndexed: true,
          },
        },
      },
    }),
  ...
  const initial: EditInitial = {
    ...
    workSince: p?.workSince ? p.workSince.toISOString().slice(0, 10) : "",
    ...
    isPublicIndexed: p?.isPublicIndexed ?? false,
    membershipStatus: user.membershipStatus ?? "student",
  }
