...
import { gameId, SCOPES, type Scope } from "@/modules/games/leaderboard";
  ...
  return (
    <div className="space-y-5">
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-[13.5px] text-gray-500">
            {winner ? "Titles won" : `Every ${cfg.name} period winner`}{scopeFilter ? ` · ${SCOPE_LABEL[scopeFilter]} only` : ""}
          </p>
        </div>
        <Link href={`/games/${cfg.slug}/leaderboard/individual/daily`} className="flex items-center gap-1.5 text-[13px] font-semibold text-brand hover:underline">
          ...
          <div className="mt-3 flex flex-wrap gap-1.5">
            {myTrophies.slice(0, 12).map((t) => (
              ...
              >
                ...
                <span className="rounded-[3px] bg-amber-100 px-1.5 py-0.5 text-[10px] text-amber-700">{PERIOD_LABEL[t.period]}</span>
              </span>
            ))}
      ...
      <div className="flex flex-wrap gap-1.5">
        <Link
          href={`/games/${cfg.slug}/champions${winner ? `?winner=${winner}` : ""}`}
        ...
        {SCOPES.map((s) => {
          const query = new URLSearchParams({ scope: s, ...(winner ? { winner } : {}) });
          return (
            <Link
              key={s}
              href={`/games/${cfg.slug}/champions?${query.toString()}`}
      ...
      {ordered.length === 0 ? (
        <div className="rounded-[5px] border border-dashed border-gray-200 bg-white p-10 text-center text-gray-500">No champions crowned yet.</div>
      ) : (
              ...
              {ordered.map((g) => (
                <tr key={`${g.period}:${g.anchor}`}>
                  <td className="px-4 py-3">
                    ...
                    <span className="ml-2 rounded-[3px] bg-gray-100 px-2 py-0.5 text-[11px] font-semibold text-gray-500">{PERIOD_LABEL[g.period]}</span>
                  </td>
                  {scopesToShow.map((s) => {
                        ...
                        {winners.length === 0 ? (
                          ...
                          <span className="flex flex-wrap items-center gap-1.5">
                            {winners.map((w) => (
                              <span
                                key={w.winnerKey}
                                className={`rounded-[3px] px-2 py-1 text-[12.5px] font-semibold ${winner && w.winnerKey === winner ? "bg-amber-100 text-amber-800" : "bg-gray-50 text-gray-800"}`}
                              >
                                {w.winnerLabel}
