...
export default async function LeaderboardPage({
  ...
  params: Promise<{ slug: string; scope: string; period: string }>;
  searchParams: Promise<{ anchor?: string }>;
}) {
  const { slug, scope: rawScope, period: rawPeriod } = await params;
  ...
  const { anchor, entries, movement } = await leaderboardWithMovementCached(cfg.key, scope, period, anchorParam);

  const rail = await Promise.all(
    SCOPES.map(async (s) => {
      const r = await leaderboardWithMovementCached(cfg.key, s, period, anchorParam);
  ...
  const base = `/games/${cfg.slug}/leaderboard`;
  ...
  return (
    <div className="space-y-5">
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="font-heading text-2xl font-bold text-gray-900">{cfg.name} Leaderboard</h1>
          <p className="text-[13.5px] text-gray-500">
            {SCOPE_LABEL[scope]} · {formatAnchor(period, anchor)}
          </p>
        </div>
        <Link href={`/games/${cfg.slug}/champions`} className="flex items-center gap-1.5 rounded-[4px] bg-amber-50 px-3.5 py-2 text-[13px] font-semibold text-amber-700 hover:bg-amber-100">
        ...
        <div className="space-y-5">
          {podium.length > 0 ? (
            ...
            <section className="rounded-[5px] border border-dashed border-gray-200 bg-white p-12 text-center">
              ...
              <Link href={`/games/${cfg.slug}/play`} className="mt-4 inline-block rounded-[4px] bg-brand px-5 py-2.5 text-sm font-semibold text-white transition-transform hover:scale-105">
                Play now
              </Link>
            </section>
          )}
              ...
              <table className="w-full text-left text-[13.5px]">
                ...
                <tbody className="divide-y divide-gray-100">
                  {(rest.length ? entries : podium).map((e: LeaderEntry) => (
                    <tr key={e.key} className={`transition-colors hover:bg-gray-50 ${e.key === user.id ? "bg-brand-50/60" : ""}`}>
                        ...
                        <span className="flex items-center gap-2">
                          {scope === "individual" &&
                            (e.avatarUrl ? (
                              ...
                              <span className="flex h-7 w-7 items-center justify-center rounded-[4px] bg-brand-50 text-[11px] font-bold text-brand">{e.label.charAt(0)}</span>
                            ))}
                          {scope === "house" && <span className="h-2.5 w-2.5 rounded-full" style={{ background: e.color ?? "#ccc" }} />}
                          <span className={`font-semibold ${e.key === user.id ? "text-brand" : "text-gray-900"}`}>{e.key === user.id ? "You" : e.label}</span>
                      ...
                      {scope === "individual" && (
                        <td className="px-4 py-3 text-right">
                          {e.bestGuesses == null ? (
                            ...
                            <span className="inline-flex items-center gap-1 rounded-[3px] bg-emerald-50 px-2 py-0.5 text-[11px] font-bold text-emerald-600" title="Ace — solved in 2 guesses or fewer">
                              <Zap className="h-3 w-3" /> Ace {e.bestGuesses}/{maxG}
                            </span>
                          ) : (
                            <span className="text-gray-500">{e.bestGuesses}/{maxG}</span>
                          )}
                        </td>
                      )}
                    </tr>
      ...
      {showNudge && nudgeTargets.length > 0 && (
        <NudgePanel connections={nudgeTargets} title="Haven't played today" subtitle="Give a connection a nudge to jump on today's board." />
      )}
    </div>
  );
}
