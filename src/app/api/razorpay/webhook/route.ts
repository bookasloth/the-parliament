...
import { issueReceiptAndInvoice } from "@/modules/membership/receipt"
...
async function handleSubscriptionActivated(payload: Record<string, unknown>) {
  ...
  if (order) {
    ...
    // First subscription charge — invoice + receipt (best-effort). The one-time
    // path is already covered inside claimAndActivateOrder (see #82).
...
async function handleSubscriptionCharged(payload: Record<string, unknown>) {
  ...
  await issueReceiptAndInvoice(chargeOrder.id)
}
