  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="border-t border-white/10 p-3 flex-shrink-0 space-y-1">
        <Link href="/feed" className="flex items-center gap-2.5 rounded-lg px-2.5 py-2 text-[13px] font-medium text-slate-400 hover:text-white hover:bg-white/5 transition-colors">
          <ExternalLink className="h-4 w-4 text-slate-500" /> View Live Site
        </Link>
        <button
          onClick={() => signOut({ callbackUrl: "/auth/signin" })}
          className="flex w-full items-center gap-2.5 rounded-lg px-2.5 py-2 text-[13px] font-medium text-slate-400 hover:text-rose-300 hover:bg-rose-500/10 transition-colors"
        >
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="relative">
              <button onClick={() => setProfileOpen(!profileOpen)} className="flex items-center gap-2 rounded-lg px-2 py-1.5 hover:bg-slate-100 transition-colors">
                <div className="flex h-8 w-8 items-center justify-center rounded-full bg-gradient-to-br from-indigo-500 to-violet-600 text-xs font-bold text-white">{admin.initials}</div>
                <div className="hidden md:block text-left">
                  <p className="text-xs font-semibold text-slate-800 leading-tight">{admin.name}</p>
              ...
              {profileOpen && (
                <div className="absolute right-0 top-full mt-1 z-30 w-48 rounded-lg border border-slate-200 bg-white py-1 shadow-lg">
                  ...
                  <Link href="/feed" onClick={() => setProfileOpen(false)} className="flex items-center gap-2.5 px-3 py-2 text-sm text-slate-600 hover:bg-slate-50">
                    <ExternalLink className="h-4 w-4" /> View Live Site
                  </Link>
                  <div className="my-1 border-t border-slate-100" />
                  <button
                    onClick={() => { setProfileOpen(false); signOut({ callbackUrl: "/auth/signin" }) }}
                    className="flex w-full items-center gap-2.5 px-3 py-2 text-sm text-rose-500 hover:bg-rose-50"
                  >
