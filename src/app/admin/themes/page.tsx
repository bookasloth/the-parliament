...
function MiniChatPreview({ theme }: { theme: ChatTheme }) {
  ...
  return (
    <div
      className="relative h-[180px] overflow-hidden rounded-lg border border-zinc-800"
...
export default function AdminThemesPage() {
  ...
  return (
    <div>
      <PageHeader
        ...
        actions={
          <>
            ...
            <button
              onClick={save}
              className={`flex items-center gap-1.5 rounded-md px-3 py-2 text-xs font-semibold text-white transition-colors ${saved ? "bg-emerald-600" : "bg-blue-600 hover:bg-blue-500"}`}
            >
      ...
      {/* Active-today banner */}
      <div className="mb-5 flex items-start gap-3 rounded-xl border border-zinc-800 bg-[#111113] p-4">
        <div className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-lg bg-blue-950/40 text-blue-400">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        {themes.map((theme) => (
          <div key={theme.id} className="rounded-xl border border-zinc-800 bg-[#111113] overflow-hidden">
            {/* Card header */}
                ...
                <div className="min-w-0">
                  <h3 className="text-sm font-bold text-zinc-100 flex items-center gap-2">
                    ...
                    {activeToday.id === theme.id && (
                      <span className="rounded-full bg-emerald-950/50 px-2 py-0.5 text-[10px] font-bold text-emerald-300 border border-emerald-800">Live</span>
                    )}
                  </h3>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-4">
              ...
              <div className="space-y-3">
                    ...
                    <>
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          className="w-full rounded-md border border-zinc-800 bg-[#0a0a0a] px-2 py-1.5 text-xs text-zinc-200 outline-none focus:border-blue-500 [color-scheme:dark]"
                        />
                ...
                <div className="rounded-lg bg-zinc-900 border border-zinc-800 p-2.5 text-[11px] text-zinc-400 space-y-1">
      ...
      <div className="mt-4 rounded-xl border border-dashed border-zinc-700 bg-[#111113] p-4">
        <div className="flex items-center gap-3 mb-3">
          ...
          <span className="ml-auto rounded-full bg-zinc-800 px-2.5 py-1 text-[10px] font-bold text-zinc-400">Always on (fallback)</span>
        </div>
      </div>
    </div>
  )
