...
import {
  DEFAULT_THEME, getActiveTheme, formatSchedule, extractThemeOverrides,
  type ChatTheme,
} from "@/config/chat-themes"
import { saveThemeOverridesAction } from "./actions"
...
export default function AdminThemesClient({ initialThemes }: { initialThemes: ChatTheme[] }) {
  const [themes, setThemes] = useState<ChatTheme[]>(initialThemes)
  ...
  function save() {
    startTransition(async () => {
      try {
        await saveThemeOverridesAction(extractThemeOverrides(themes))
        setSaved(true)
        setTimeout(() => setSaved(false), 2500)
      } catch {
        toast.error("Could not save theme schedule. Try again.")
      }
    })
  }
        ...
        actions={
          <>
            ...
            <button
              onClick={save}
              disabled={pending}
              className={`flex items-center gap-1.5 rounded-[3px] px-3 py-2 text-xs font-semibold text-white transition-colors disabled:opacity-60 ${saved ? "bg-emerald-600" : "bg-blue-600 hover:bg-blue-500"}`}
            >
              {saved ? <><CheckCircle className="h-3.5 w-3.5" weight="duotone" /> Saved</> : <><FloppyDisk className="h-3.5 w-3.5" weight="duotone" /> {pending ? "Saving…" : "Save Schedule"}</>}
            </button>
          </>
        }
      />
