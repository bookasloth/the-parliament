  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-5">
        <StatCard label="Total Users" value={(stats?.total ?? users.length).toLocaleString()} icon={<Users className="h-4.5 w-4.5" />} accent="indigo" />
        <StatCard label="Verified" value={(stats?.verified ?? 0).toLocaleString()} icon={<UserCheck className="h-4.5 w-4.5" />} accent="emerald" />
        <StatCard label="Pending Approval" value={(stats?.pending ?? 0).toLocaleString()} icon={<Clock className="h-4.5 w-4.5" />} accent="amber" />
        <StatCard label="Suspended" value={(stats?.suspended ?? 0).toLocaleString()} icon={<UserX className="h-4.5 w-4.5" />} accent="rose" />
      </div>
      ...
      <div className="rounded-xl border border-slate-200 bg-white overflow-hidden">
        ...
        <div className="flex items-center justify-between px-4 py-3 border-t border-slate-100">
          <p className="text-xs text-slate-400">Showing <span className="font-semibold text-slate-600">{filtered.length}</span> of <span className="font-semibold text-slate-600">{(stats?.total ?? users.length).toLocaleString()}</span> users</p>
      ...
      {inviteOpen && (
        <div role="presentation" className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={() => setInviteOpen(false)}>
