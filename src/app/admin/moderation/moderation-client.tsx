  ...
  return (
    <div>
      ...
      <div className="flex gap-1.5 flex-wrap mb-4">
        {(["all", "post", "comment", "profile", "business", "message"] as TypeFilter[]).map((t) => (
          <button key={t} onClick={() => setTypeFilter(t)}
            className={`rounded-[3px] border px-3 py-1 text-[11px] font-semibold capitalize transition-colors ${typeFilter === t ? "border-blue-800 bg-blue-950/50 text-blue-300" : "border-zinc-800 bg-[#111113] text-zinc-500 hover:border-zinc-700"}`}>
            {t === "all" ? "All Types" : `${t}s`}
          </button>
        ))}
      </div>
      ...
      <div className="space-y-3">
        {list.length === 0 && (
          <div className="rounded-[5px] border border-zinc-800 bg-[#111113] py-16 text-center">
        ...
        {list.map((report) => {
          ...
          return (
            <div key={report.id} className="rounded-[5px] border border-zinc-800 bg-[#111113] overflow-hidden">
              <button onClick={() => setExpanded(isOpen ? null : report.id)} className="flex w-full items-start gap-3 p-4 text-left hover:bg-zinc-900/50 transition-colors">
                <div className="flex h-9 w-9 items-center justify-center rounded-[4px] flex-shrink-0 bg-zinc-800 text-zinc-400">
                  {typeIcons[report.entityType] ?? <Flag className="h-4 w-4" weight="duotone" />}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap mb-0.5">
                    <span className="rounded-[3px] bg-zinc-800 px-2 py-0.5 text-[10px] font-semibold capitalize text-zinc-400">{report.entityType}</span>
                  </div>
              ...
              {isOpen && (
                <div className="border-t border-zinc-800 p-4 sm:p-5">
                  <div className="rounded-[4px] border border-zinc-800 bg-zinc-900 p-4 mb-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <button onClick={() => act(report.id, "hidden", "Content hidden")} disabled={pending}
                      className="flex items-center gap-1.5 rounded-[3px] border border-amber-800 bg-amber-950/50 px-3.5 py-2 text-xs font-bold text-amber-300 hover:bg-amber-950 disabled:opacity-50">
                      <EyeSlash className="h-3.5 w-3.5" weight="duotone" /> Hide
                    </button>
                    <button onClick={() => act(report.id, "warned", "User warned")} disabled={pending}
                      className="flex items-center gap-1.5 rounded-[3px] border border-zinc-700 px-3.5 py-2 text-xs font-bold text-zinc-200 hover:bg-zinc-800 disabled:opacity-50">
                      <Warning className="h-3.5 w-3.5" weight="duotone" /> Warn
                    </button>
                    <button onClick={() => act(report.id, "dismissed", "Dismissed")} disabled={pending}
                      className="flex items-center gap-1.5 rounded-[3px] border border-zinc-700 px-3.5 py-2 text-xs font-semibold text-zinc-400 hover:bg-zinc-800 disabled:opacity-50 ml-auto">
