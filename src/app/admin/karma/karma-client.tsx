    ...
    <div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        <div className="xl:col-span-2 space-y-4">
            ...
            <div className="divide-y divide-zinc-800">
              {thresholds.map((t, i) => (
                <div key={t.level} className="flex items-center gap-3 px-4 sm:px-5 py-3">
                  <div className="flex h-8 w-8 items-center justify-center rounded-[4px] bg-violet-950/40 text-violet-400 text-xs font-bold flex-shrink-0">{i + 1}</div>
          ...
          <div className="rounded-[5px] border border-zinc-800 bg-[#111113] p-4 sm:p-5">
            ...
            <div className="space-y-2.5">
              {caps.map((c) => (
                <div key={c.label} className="flex items-center gap-2">
                  ...
                  <span className="rounded-[3px] bg-zinc-800 px-2 py-0.5 text-xs font-bold text-zinc-300 tabular-nums">{c.value}</span>
                </div>
              ))}
              <div className="flex items-center gap-2 rounded-[4px] bg-zinc-900 border border-zinc-800 p-3 mt-3">
                ...
                <span className="rounded-[3px] bg-zinc-800 px-2 py-0.5 text-xs font-bold text-zinc-300">0</span>
              </div>
            </div>
          </div>
        </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="e.g. amit-verma or amit@…"
                    className="w-full rounded-[3px] border border-zinc-800 bg-[#0a0a0a] pl-8 pr-3 py-2 text-xs text-zinc-200 placeholder-zinc-500 outline-none focus:border-blue-500" />
                </div>
              </div>
              <div>
                <label className="block text-[10px] font-bold uppercase tracking-wide text-zinc-500 mb-1">Amount</label>
                <div className="flex gap-1.5">
                  <button onClick={() => setAmount((a) => String(Math.abs(Number(a) || 25)))} className="flex h-8 w-8 items-center justify-center rounded-[3px] border border-emerald-800 text-emerald-400 hover:bg-emerald-950/50"><Plus className="h-3.5 w-3.5" weight="duotone" /></button>
                  <button onClick={() => setAmount((a) => String(-Math.abs(Number(a) || 25)))} className="flex h-8 w-8 items-center justify-center rounded-[3px] border border-rose-800 text-rose-400 hover:bg-rose-950/50"><Minus className="h-3.5 w-3.5" weight="duotone" /></button>
                  <input value={amount} onChange={(e) => setAmount(e.target.value)} type="number" placeholder="e.g. 50 or -25"
                    className="flex-1 rounded-[3px] border border-zinc-800 bg-[#0a0a0a] px-3 py-2 text-xs font-bold text-zinc-200 placeholder-zinc-500 tabular-nums outline-none focus:border-blue-500" />
                </div>
              </div>
              <div>
                ...
                <textarea value={reason} onChange={(e) => setReason(e.target.value)} rows={2}
                  ...
                  className="w-full rounded-[3px] border border-zinc-800 bg-[#0a0a0a] px-3 py-2 text-xs text-zinc-200 placeholder-zinc-500 outline-none focus:border-blue-500 resize-none" />
              </div>
              {msg && (
                <div className={`rounded-[3px] px-3 py-2 text-xs font-semibold ${msg.ok ? "bg-emerald-950/50 border border-emerald-800 text-emerald-300" : "bg-rose-950/40 border border-rose-900 text-rose-300"}`}>{msg.text}</div>
              )}
              ...
              <ul className="divide-y divide-zinc-800">
                {earners.map((u, i) => (
                  <li key={i} className="flex items-center gap-3 px-4 py-2.5">
                    <span className={`flex h-6 w-6 items-center justify-center rounded-[4px] text-[10px] font-bold flex-shrink-0 ${i === 0 ? "bg-amber-950/50 text-amber-300" : i === 1 ? "bg-zinc-800 text-zinc-300" : i === 2 ? "bg-orange-950/50 text-orange-300" : "bg-zinc-900 text-zinc-500"}`}>{i + 1}</span>
